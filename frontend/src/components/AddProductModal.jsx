...
import { createProduct, getCategories } from "../util/Http";
...
export default function AddProductModal({ isModalOpen, setIsModalOpen }) {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (formData) => createProduct(token, formData),
    onSuccess: (data) => {
      if (data.status === "success") {
        ...
        notifyError(data?.response?.data?.message || "Error adding product!");
      }
    },
    onError: () => {
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                      ...
                      <div className="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
                        <button
                          ...
                          disabled={isPending}
                        >
                          {isPending ? <Spinner size={5} /> : "Add product"}
                        </button>
                        <button
                          type="button"
