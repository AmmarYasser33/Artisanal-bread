...
import { addTestimonial } from "../util/Http";
import { IconBadge } from "../Icons";
import ImageUploader from "./ImageUploader";
import Spinner from "./Spinner";
import { BASE_URL } from "../util/Globals";

export default function AddTestimonialModal({ isModalOpen, setIsModalOpen }) {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (formData) => addTestimonial(token, formData),
    onSuccess: (data) => {
      if (data.status === "success") {
        ...
        notifyError(
          data?.response?.data?.message || "Error adding testimonial!",
        );
      }
    },
    onError: () => {
      notifyError("Error adding testimonial!");
    },
  });
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                      <div className="mt-5 bg-white">
                        <div className="px-5 pb-5">
                          ...
                          <ImageUploader
                            ...
                            initialImages={[
                              `${BASE_URL}testimonials/default.png`,
                            ]}
                          />
                        </div>
                      ...
                      <div className="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
                        <button
                          ...
                          disabled={isPending}
                        >
                          {isPending ? <Spinner size={5} /> : "Add testimonial"}
                        </button>
                        <button
                          type="button"
