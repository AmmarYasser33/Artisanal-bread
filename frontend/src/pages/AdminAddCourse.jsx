...
import { createCourse } from "../util/Http";
...
export default function AdminCourse() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (formData) => createCourse(token, formData),
    onSuccess: (data) => {
      if (data.status === "success") {
        navigate("/admin/courses");
        notifySuccess("Course added successfully");
      } else {
        notifyError(data?.response?.data?.message || "Failed to add course!");
      }
    },
    onError: () => {
          ...
          >
            <div className="grid grid-cols-6 gap-6">
              ...
              <div className="col-span-6 flex items-center gap-2 sm:col-span-3">
                ...
                {errors.isOnline && (
                  <span className="text-sm text-red-600">
                    Select if the course is online or not
                  </span>
                )}
              </div>
